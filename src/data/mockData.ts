...
- All notification types and channels documented`,
  ...
  {
    id: 'task-007',
    title: generateTitle('Design REST API for payment processing with Stripe integration'),
    input: 'Design REST API for payment processing with Stripe integration',
    taskType: 'API_DESIGN',
    status: 'success',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'API Design & Spec', description: 'Endpoints, schemas, error handling, Stripe webhook integration', estimatedTime: '50 min' },
      { session: 2, title: 'Documentation', description: 'OpenAPI spec, examples, idempotency keys, retry strategy', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'All payment endpoints documented (/charges, /refunds, /webhooks)',
      'Request/response schemas defined with examples',
      'Error codes documented (402, 401, 429, 500)',
      'Idempotency keys for payment mutation safety',
      'Webhook signature verification documented',
    ],
  },
  {
    id: 'task-008',
    title: generateTitle('Security audit: Review authentication system for vulnerabilities'),
    input: 'Security audit: Review authentication system for vulnerabilities',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Security Review', description: 'Code review, threat modeling, OWASP Top 10 assessment', estimatedTime: '90 min' },
      { session: 2, title: 'Remediation Plan', description: 'Prioritize findings, create fix roadmap, risk matrix', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'No hardcoded secrets or API keys found',
      'Authentication flow reviewed (JWT, sessions, refresh tokens)',
      'All findings documented with severity and fix',
      'Critical findings have immediate remediation',
      'Password storage reviewed (hashing, salt)',
    ],
  },
  {
    id: 'task-009',
    title: generateTitle('Create test strategy for mobile app with 80% coverage goal'),
    input: 'Create test strategy for mobile app with 80% coverage goal',
    taskType: 'TEST_STRATEGY',
    status: 'pending',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Test Planning', description: 'Define test pyramid, coverage targets, critical paths', estimatedTime: '45 min' },
      { session: 2, title: 'Automation Setup', description: 'Jest/Detox setup, test infrastructure, sample tests', estimatedTime: '45 min' },
    ],
    generatedChecklist: [
      'Test pyramid defined: unit/integration/e2e ratio',
      '80% coverage target for business logic',
      'Critical user flows have E2E tests',
      'CI/CD integration plan for test automation',
      'Performance testing approach defined',
    ],
  },
  {
    id: 'task-010',
    title: generateTitle('Architecture: Should we migrate from monolith to microservices?'),
    input: 'Architecture: Should we migrate from monolith to microservices?',
    taskType: 'ARCH_DECISION',
    status: 'success',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Architecture Design', description: '2-3 options analysis, trade-off matrix, selected architecture', estimatedTime: '90 min' },
      { session: 2, title: 'Implementation Plan', description: 'Phased roadmap, technology stack, deployment strategy', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'Monolith vs microservices vs hybrid options analyzed',
      'Cost-benefit analysis with realistic numbers',
      'Team skills assessment for chosen architecture',
  ...
  {
    id: 'task-011',
    title: generateTitle('Setup monitoring and alerting for production infrastructure'),
    input: 'Setup monitoring and alerting for production infrastructure',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Observability Design', description: 'Metrics, alerts, logging strategy, dashboard design', estimatedTime: '40 min' },
      { session: 2, title: 'Infrastructure Setup', description: 'Prometheus/Grafana setup, alert rules, incident playbooks', estimatedTime: '45 min' },
    ],
    generatedChecklist: [
      'Key metrics defined: latency, throughput, error rate, resource usage',
      'Alert rules configured (not noisy)',
      'Logging strategy covers failure scenarios',
      'Dashboards for ops, business, and development views',
      'SLO/SLI targets defined and tracked',
    ],
  },
  {
    id: 'task-012',
    title: generateTitle('Code review: Payment checkout flow changes before merge'),
    input: 'Code review: Payment checkout flow changes before merge',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Code Review', description: 'Security, correctness, performance review of checkout changes', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'No exposed secrets or hardcoded credentials',
      'Error handling covers all edge cases',
      'Security: input validation, CSRF tokens, PCI compliance',
  ...
  {
    id: 'task-013',
    title: generateTitle('Refactor: Simplify deeply nested component tree in dashboard'),
    input: 'Refactor: Simplify deeply nested component tree in dashboard',
    taskType: 'REFACTOR',
    status: 'pending',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Characterize & Test', description: 'Map component tree, write characterization tests', estimatedTime: '30 min' },
      { session: 2, title: 'Refactor', description: 'Extract components, reduce nesting, simplify prop drilling', estimatedTime: '45 min' },
    ],
    generatedChecklist: [
      'Characterization tests written before refactoring',
      'All existing tests pass after refactor',
      'No observable behavior change',
      'Component nesting reduced by 50%',
      'Prop drilling eliminated for deep components',
    ],
  },
  {
    id: 'task-014',
    title: generateTitle('Debug: Why is the cron job running multiple times concurrently?'),
    input: 'Debug: Why is the cron job running multiple times concurrently?',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Investigate & Report', description: 'Hypothesis testing, root cause analysis, no fixes yet', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'Root cause identified: distributed lock missing or timeout too short',
      'Evidence collected: logs showing overlapping executions',
      'Recommended fix: implement Redis locks or database mutex',
      'Test scenario reproduces the issue consistently',
    ],
  },
  {
    id: 'task-015',
    title: generateTitle('Integrate: Connect weather API to update dashboard in real-time'),
    input: 'Integrate: Connect weather API to update dashboard in real-time',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Mock-First Integration', description: 'Define types, build UI with mock data', estimatedTime: '30 min' },
      { session: 2, title: 'Real Integration', description: 'Wire real API, add error handling, caching', estimatedTime: '30 min' },
    ],
    generatedChecklist: [
      'TypeScript types for weather data defined',
      'UI renders correctly with mock weather data',
      '[MOCK] badge visible when using mock data',
      'Real API integration works with error handling',
      'Response caching to reduce API calls',
    ],
  },
  {
    id: 'task-016',
    title: generateTitle('Memory leak in background worker consuming 2GB RAM after 24hrs'),
    input: 'Memory leak in background worker consuming 2GB RAM after 24hrs',
    taskType: 'BUG_FIX',
    status: 'pending',
    ...
    generatedFiles: [],
    generatedPlan: [
      { session: 1, title: 'Diagnose & Fix', description: 'Use heap profiling, identify leak source, minimal fix', estimatedTime: '45 min' },
    ],
    generatedChecklist: [
      'Heap dump captured and analyzed',
      'Leak source identified: likely event listeners, timers, or object references',
      'Memory profiling before/after fix shows stable memory',
      'Regression test for long-running process',
    ],
  },
];
